...
def mom_upad_input(
    ...
    q_dim = query_layer.shape[-1]
    batch_size, kv_seq_len, k_dim = key_layer.shape
    v_dim = value_layer.shape[-1]

    key_layer = index_first_axis(key_layer.reshape(batch_size * kv_seq_len, k_dim), indices_k)
    ...
    if query_length == kv_seq_len:
        query_layer = index_first_axis(query_layer.reshape(batch_size * kv_seq_len, q_dim), indices_k)
...
class MomAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        cu_q, cu_k, cu_v, cu_g, cu_beta, indices_q, cu_seqlen_all, max_seq_lens = mom_upad_input(q, k, v, g, beta, mask_2)
